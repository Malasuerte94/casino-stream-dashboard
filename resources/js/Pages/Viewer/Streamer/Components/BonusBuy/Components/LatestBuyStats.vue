...
export default {
  ...
  computed: {
    totalBonuses() {
      return this.latestBuy?.bonus_buy_games.length || 0;
    },
    openedBonuses() {
      return this.latestBuy?.bonus_buy_games.filter(game => Number(game.result)).length || 0;
    },
    currentTotalWin() {
      return this.latestBuy?.bonus_buy_games.reduce((sum, game) => sum + Number(game.result) || 0, 0);
    },
    breakEvenX() {
      const listCost = this.startBalance;
      const totalPayout = this.latestBuy?.bonus_buy_games.reduce(
          (sum, game) => sum + (game.result ? parseFloat(game.result) : 0),
          0
      ) || 0;

      const remainingCost = Math.max(listCost - totalPayout, 0);
      const remainingGames = this.latestBuy?.bonus_buy_games.filter(
      ...
      this.latestBuy?.bonus_buy_games.forEach((game) => {
    ...
    currentX() {
      const totalBets = this.latestBuy?.bonus_buy_games.reduce((sum, game) => sum + (Number(game.stake) || 0), 0);
      return totalBets > 0 ? (this.currentTotalWin / totalBets).toFixed(2) : 0;
    },
    startBalance() {
      return this.latestBuy?.start || 0;
    },
    avgBet() {
      return this.totalBonuses > 0
          ? (this.latestBuy?.bonus_buy_games.reduce((sum, game) => sum + (Number(game.stake) || 0), 0) / this.totalBonuses).toFixed(2)
          : 0;
    }
  },
  methods: {
