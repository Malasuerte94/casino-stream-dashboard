<template>
      ...
      <div v-for="(winner, key) in winners" :key="key"
        ...
        <div class="flex flex-col">
          ...
          <div v-if="key === 'resultWinner'">
            ...
            <span :class="getDifferenceClass(winner.estimated, buyResults.result)">
              ({{ formatDifference(winner.estimated, buyResults.result) }})
            </span>
          </div>
          <div v-else-if="key === 'biggestMultiplierWinner' || key === 'lowestMultiplierWinner'">
            <span>{{ formatMultiplier(winner.estimated) }}</span>
            <span :class="getDifferenceClass(winner.estimated, buyResults[key.replace('Winner', '')])">
              ({{ formatMultiplierDifference(winner.estimated, buyResults[key.replace('Winner', '')]) }})
            </span>
          </div>
  ...
  <div class="relative" v-if="latestBuy">
    <!-- Main Form -->
    <div
    ...
    >
      ...
      <div class="font-semibold">{{$t('buy_predictions')}}</div>

      <!-- Multiplier Inputs (Grouped) -->
      <div class="grid grid-cols-2 gap-4">
        <MyInput label="Cel mai mare multi" v-model="highestMulti" type="number" />
        <MyInput label="Cel mai mic multi" v-model="lowestMulti" type="number" />
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col gap-1">
          ...
          >
            <option v-for="game in latestBuy.bonus_buy_games" :key="game.id" :value="game.id">
              {{ game.game.name }}
            </option>
          </select>
        </div>
    ...
    <div
        v-if="!$page.props.user.id || !latestBuy.is_open"
    ...
    >
      <div class=" p-1 rounded-lg text-white">
        ...
        <div v-else class="text-sm font-semibold text-left">{{$t('no_prediction_found')}}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchExistingPrediction() {
      try {
        const response = await axios.get(`/api/viewer/get-bbl-predictions/${this.latestBuy.id}`);
        ...
        } else if(!this.latestBuy.is_open) {
    ...
    async fetchWinners() {
      try {
        const response = await axios.get(`/api/get-bonus-winner/${this.latestBuy.id}`);
        if(response.data && response.data.winners) {
          this.winners = response.data.winners;
          this.buyResults = response.data.results;
        } else {
          this.winners = null;
          this.buyResults = null;
        }
      } catch (error) {
    ...
    getGameName(gameId) {
      const game = this.latestBuy.bonus_buy_games.find(g => g.id === gameId);
