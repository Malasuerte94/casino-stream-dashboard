<template>
  <div
      ...
      ref="scrollContainer">
    <div v-for="bonusBuy in bonusBuyHistory" @click="seeBuy(bonusBuy)"
      ...
      <div class="flex flex-row justify-between items-center">
        <div class="text-sm">
          {{$t('bonus_buy')}} #{{ bonusBuy.id }}
        </div>
        <div class="bg-gray-800 px-2 py-1 rounded text-xs"
             :class="{ 'text-gray-500': bonusBuy.ended, 'text-green-500': !bonusBuy.ended }">
          {{ bonusBuy.ended ? 'TERMINAT' : 'ACTIV' }}
        </div>
      </div>
      <div class="flex flex-row justify-between items-center text-xl">
        <div class="flex flex-row items-center gap-1 bg-gray-700 px-2 py-1 rounded">
          ...
          <div>{{ bonusBuy.bonus_buy_games.length }}</div>
        </div>
        <div class="flex flex-row items-center gap-1 bg-gray-700 px-2 py-1 rounded">
          <div>
            <SvgMoney class="h-5 w-5 text-green-500"/>
          </div>
          <div>{{ bonusBuy.start }} {{ currency }}</div>
        </div>
      </div>
      <div class="flex flex-row items-center justify-between gap-1 text-sm">
        <div class="flex flex-row items-center gap-1">
          ...
          <div>{{ convertDate(bonusBuy.created_at) }}</div>
        </div>
        <div
            class="flex flex-row items-center gap-1 bg-gray-700 px-2 py-1 rounded"
            :class="{
              'text-green-500': parseFloat(bonusBuy.result) >= parseFloat(bonusBuy.start),
              'text-red-500': parseFloat(bonusBuy.result) <= parseFloat(bonusBuy.start)
            }"
        >
          <div>
            <SvgMoney class="h-4 w-4"/>
          </div>
          <div>{{ bonusBuy.result }} {{ currency }}</div>
        </div>
      </div>
    </div>
  </div>
