...
export default {
  ...
  computed: {
    highestWinGame() {
      if (!this.latestBuy?.bonus_buy_games?.length) return null;
      return this.latestBuy.bonus_buy_games.reduce((max, game) =>
              Number(game.result) > Number(max.result) ? game : max
          , this.latestBuy.bonus_buy_games[0]);
    },
    lowestWinGame() {
      if (!this.latestBuy?.bonus_buy_games?.length) return null;
      return this.latestBuy.bonus_buy_games.reduce((min, game) =>
