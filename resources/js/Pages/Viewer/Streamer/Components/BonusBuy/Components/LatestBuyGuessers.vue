<template>
  <div class="relative" v-if="latestBuy && allGuessers.length > 0">
    <div
    ...
    >
      <div class="font-semibold">Participanți</div>
...
export default {
  ...
  methods: {
    async fetchExistingPredictions() {
      try {
        const response = await axios.get(`/api/viewer/get-bh-predictions/${this.latestBuy.id}`);
