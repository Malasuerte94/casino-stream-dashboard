<template>
            ...
            <div class="grid grid-cols-[minmax(200px,_1fr)_120px] gap-2 mt-2 mb-2">
                <input @input="startTimerUpdateBonusHunt" type="text" id="bonus_Hunt_name" v-model="bonusHunt.name"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Nume Lista">
                <button @click="resetBonusHunt" type="button" tabindex="-1"
            ...
            <template v-if="bonusHuntGames">
                ...
                <div v-for="(game, index) in bonusHuntGames" :key="index"
                    class="grid grid-cols-[30px_minmax(200px,_1fr)_120px_120px_120px_120px_40px] gap-2 mt-4">
                    <div
                        class="w-8 h-8 justify-center self-center text-white bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:focus:ring-blue-800">
                        {{ index }}
                    </div>
                    <div><input @input="checkModifiedFields(game, index)" v-model="game.name" type="text" id="name"
                            class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Game Name" /></div>
                    <div><input @input="checkModifiedFields(game, index)" v-model="game.stake" type="number" id="stake"
                    ...
                    <div><input @input="checkModifiedFields(game, index)" v-model="game.price" type="number" id="price"
                            class="text-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Price" /></div>
                    <div><input @input="checkModifiedFields(game, index)" v-model="game.result" type="number"
                            id="result"
                            class="text-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="Result" /></div>
                    <div><input disabled v-model="game.multiplier" type="number" id="multiplier"
                ...
                <div class="flex mt-8 center justify-center">
                    <button @click="createNewBonusHuntGameRow" type="button"
...
export default {
    ...
    methods: {
        async getLatestList() {
            await axios.get('/api/bonus-hunt')
                .then(response => {
        ...
        async forceUpdateBonusHuntGames() {
            ...
            await axios.put('/api/bonus-hunt-games', {
                games,
            })
        ...
        async removeBonusHuntGameRow(id) {
            await axios.delete('/api/bonus-hunt-games/' + id)
                .then(response => {
                    this.bonusHuntGames = this.bonusHuntGames.filter(game => game.id != id)
                })
                .catch(error => {
                    console.log(error)
                })
        ...
        async updateBonusHunt() {
            ...
            await axios.patch('/api/bonus-hunt', {
                bonusHunt,
            })
                .then(function (response) {
                    console.log(response);
        ...
        calcCurentRowMultiplier(game, index) {
            ...
            this.bonusHuntGames[index].multiplier = Math.round(game.result / game.stake, 2)
        },
    }
}
</script>
